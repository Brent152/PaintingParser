...
def create_color_visualization(index_map, colors, cell_size=20, show_numbers=True):
    ...
    try:
        ...
        try:
            ...
            font = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", max(8, cell_size // 3))
        except:
...
def create_color_palette_legend(colors, cell_size=40, cols=4):
    ...
    legend_width = cols * (cell_size + text_width + 20)  # 20px margin
    legend_height = rows * (cell_size + 10) + 20  # 10px spacing, 20px top margin
    ...
    for index in sorted(colors.keys()):
        ...
        # Calculate position
        x_base = current_col * (cell_size + text_width + 20) + 10
        y_base = current_row * (cell_size + 10) + 10
        ...
        # Index and name
        draw.text((text_x, text_y), f"Index {index}: {color_data['name']}", fill='black', font=font)
        draw.text((text_x, text_y + 12), f"Count: {color_data['count']} pixels", fill='black', font=font)
        
        # CMYK values
        c, m, y, k = color_data['cmyk']
        draw.text((text_x, text_y + 24), f"CMYK: {c:.2f}, {m:.2f}, {y:.2f}, {k:.2f}", fill='black', font=font)
